...
class Trainer:
    ...
    def save_checkpoint(self, epoch):
        ...
        # 分离路径和文件名
        dir_name, file_name = os.path.split(self.config.ckpt_path)
        # 在文件名前添加 epoch
        new_file_name = f"epoch_{epoch}_{file_name}"
        # 重新组合路径和新的文件名
        ckpt_path = os.path.join(dir_name, new_file_name)

        logger.info("saving %s", ckpt_path)
        torch.save(raw_model.state_dict(), ckpt_path)


    def train(self, wandb, generate = False):
            ...
            for it, (x, y, p, scaffold) in pbar:
                ...
                with torch.cuda.amp.autocast():
                    with torch.set_grad_enabled(is_train):
                        logits = model(x, p, scaffold,num_last_tokens = config.block_size - 1).logits
        ...
        if generate:     
            ...
            tinib_data = pd.read_csv('datasets/' + 'tinib_random' + '.csv')
            valid_ratio,uni_ratio,similarity = g_test(self.model,tinib_data,self.test_dataset.max_len,
                                           self.test_dataset.scaf_max_len,self.stoi,self.itos)
            
        return best_loss,valid_ratio,uni_ratio,similarity
